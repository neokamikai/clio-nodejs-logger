...
import loggerLevels from '../../../src/levels';
import LogLevel from '../../../src/@types/types/log-level';
...
describe('Logger', () => {
  context('with formatter', () => {
    ...
    function createLoggerWithLogFormat(logFormat: string) {
      ...
      return [logger, spy(logger, 'format')] as [Logger, Sinon.SinonSpy<any[], any>];
    }
  ...
  context('suppressing output via log level', () => {
    function createLoggerWithLogLevel(logLevel: any, extraParameters = {}) {
      const logger = new Logger({
        context: {},
        logLevel,
        logLimit: 7000,
      ...
      return [logger, spy(logger, 'format')] as [Logger, Sinon.SinonSpy];
    }

    const loggerMethods: Array<LogLevel | 'info'> = ['debug', 'info', 'warn', 'error'];
